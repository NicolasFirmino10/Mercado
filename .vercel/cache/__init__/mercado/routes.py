    ...
    if request.method == 'POST':
        ...
        if produto_obj:
            if current_user.compra_disponivel(produto_obj):
                ...
                flash(f'Você não tem dinheiro suficiente para comprar o produto {produto_obj.nome}!', category = 'danger')
...
def page_cadastro():
    ...
    if form.errors != {}:
        for err in form.errors.values():
            flash(f'Erro de validação: {err}', category = 'danger')
...
def page_logout():
    logout_user()
    flash('Logout feito com sucesso!', category = 'info')
